...
const NotificationError = (error) => {
  ...
  //Login
    ...
    NotificationStore(
      "Erro",
      "O usuário não tem acesso a essa aplicação",
      "danger"
    );
  ...
  //Media
    ...
    error.includes("(id, description, file, type, supplier_id) is broken")
  )
    NotificationStore(
      "Erro",
      "Preencha todos os campos",
    ...
    NotificationStore(
      "Erro",
      "Essa mídia esta em uso e não pode ser modificada ",
      "danger"
    );
    ...
    NotificationStore(
      "Erro",
      "Tipo 0, 1 and 2 são apenas para mídia interna",
      "danger"
    );
  else if (error.includes("Type 3 and 4 is only to external"))
    NotificationStore(
      "Erro",
      "Tipo 3 and 4 são apenas para mídia externa",
      "danger"
    );
  //ProductScreen
    ...
    error.includes(
      "The media type on this screen does not allow a products list"
    )
  )
    NotificationStore(
      "Erro",
      "O tipo de mídia nesta tela não permite uma lista de produtos",
      "danger"
    );
  //Screen
  else if (
    error.includes("(description, time, media_id, department_id) is broken")
  )
    NotificationStore(
      "Erro",
      "Preencha todos os campos",
    ...
    error.includes("(id, description, time, media_id, department_id) is broken")
  )
    NotificationStore(
      "Erro",
      "Preencha todos os campos",
    ...
    NotificationStore(
      "Erro",
      "O tipo 0 que corresponde à lista de produtos não pode ser configurado sem departamento",
      "danger"
    );
  else if (
    error.includes(
      "Department can only be configurated if the product list is type 0"
    )
  )
    NotificationStore(
      "Erro",
      "Departamento só pode ser configurado se a lista de produtos for do tipo 0",
      "danger"
    );
  //Supplier
  ...
  //Timeline
    ...
    error.includes(
      "(description, initial_hour, final_hour, initial_date, final_date) is broken"
    )
  )
    NotificationStore(
      "Erro",
    ...
    error.includes(
      "(id, description, initial_hour, final_hour, initial_date, final_date) is broken"
    )
  )
    NotificationStore(
      "Erro",
    ...
    error.includes(
      "Incorrect format for initial hour. The correct format is (00:00:00)"
    )
  )
    NotificationStore(
      "Erro",
      "Formato incorreto para a hora inicial. O formato correto é (00:00:00)",
      "danger"
    );
  else if (
    error.includes(
      "Incorrect value for final hour. The correct format is (00:00:00)"
    )
  )
    NotificationStore(
      "Erro",
      "Valor incorreto para hora final. O formato correto é (00:00:00)",
      "danger"
    );
  else if (
    error.includes("Initial hour could not be major or equals the final hour")
  )
    NotificationStore(
      "Erro",
    ...
    error.includes(
      "Incorrect format for initial date. The correct format is (1990-01-01)"
    )
  )
    NotificationStore(
      "Erro",
      "Formato incorreto para data inicial. O formato correto é (1990-01-01)",
      "danger"
    );
  else if (
    error.includes(
      "Incorrect value for final date. The correct format is (1990-01-01)"
    )
  )
    NotificationStore(
      "Erro",
      "Valor incorreto para data final. O formato correto é (1990-01-01)",
      "danger"
    );
